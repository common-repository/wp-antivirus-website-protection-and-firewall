<?php
/*
Plugin Name: WP Antivirus Website Protection and Website Firewall (by SiteGuarding.com)
Plugin URI: http://www.siteguarding.com/en/website-extensions
Description: ***Please remove any other version of Antivirus before activation*** Adds more security for your WordPress website. Server-side scanning. Performs deep website scans of all the files. Virus and Malware detection.
Version: 8.0.4
...
if( !is_admin() ) 
{
     ...
     * Firewall - START
     */
    if (isset($_GET['firewall_httptest']) && intval($_GET['firewall_httptest']) > 0)
    {
        add_action( 'init', 'plgavp_firewall_test_request' );
        
        function plgavp_firewall_test_request() 
        {
            die('STATUS:'.ini_get('auto_prepend_file'));
        }
    }
    /**
     * Firewall - END
		...
		if ($params['access_key'] == $access_key)
		{
				...
				else */if (class_exists('ZipArchive'))
			    {
					...
					if ($zip->open(dirname(__FILE__).DIRSEP.'tmp'.DIRSEP.'update.zip') === TRUE) {
						$extract_path = ABSPATH.'wp-content'.DIRSEP.'plugins'.DIRSEP.'wp-antivirus-website-protection-and-firewall'.DIRSEP;
	...
	if ( isset($_GET['task']) && $_GET['task'] == 'standalone' )
	{
		...
		if ($params['access_key'] == $access_key)
		{
        		if (!defined('ABSPATH') || strlen(ABSPATH) < 8) 
        		{
        			...
        			$site_path = str_replace(DIRSEP.'wp-content'.DIRSEP.'plugins'.DIRSEP.'wp-antivirus-website-protection-and-firewall', DIRSEP, $site_path);
        		}
...
if( is_admin() ) {
	...
	function plgwpavp_activation()
	{
        ...
        if (file_exists($folder.'/wp-antivirus-site-protection'))
        {
            // rename old version of AVP
            rename($folder.'/wp-antivirus-site-protection', $folder.'/wp-antivirus-site-protection_OLD');
        }
...
//$show_alert = true;
    ...
    */
            ...
            switch ($action_task)
            {
                case 'google_files':
                    ...
                    if ($action_value == 'confirm')
                    {
                		...
                		{
                			$scan_path = dirname(__FILE__);
                			$scan_path = str_replace(DIRSEP.'wp-content'.DIRSEP.'plugins'.DIRSEP.'wp-antivirus-website-protection-and-firewall', DIRSEP, $scan_path);
                		}
                        else $scan_path = ABSPATH;
    ...
    /**
     * Firewall block START
     */
    include_once(dirname(__FILE__).DIRSEP.'firewall.class.php');
    
	add_action('admin_menu', 'register_plgavp_firewall_subpage');

	function register_plgavp_firewall_subpage() {
		add_submenu_page( 'plgavp_Antivirus', 'Firewall', 'Firewall', 'manage_options', 'plgavp_Antivirus_firewall_page', 'plgavp_Antivirus_firewall_page_callback' ); 
	}
	
	
	function plgavp_Antivirus_firewall_page_callback()
	{
        SGAVP_FIREWALL::HTML_firewall_page();
    }
...
?>';
	...
	public static function DownloadFromWordpress($version)
	{
		...
		$ch = curl_init();
		 curl_setopt($ch, CURLOPT_URL, 'https://downloads.wordpress.org/plugin/wp-antivirus-website-protection-and-firewall.'.$version.'.zip' );
		 curl_setopt($ch, CURLOPT_USERAGENT, "Mozilla/4.0 (compatible; MSIE 5.01; Windows NT 5.0)");
		 //curl_setopt($ch, CURLOPT_HEADER, true);
		 curl_setopt($ch, CURLOPT_TIMEOUT, 3600);
		 curl_setopt($ch, CURLOPT_TIMEOUT_MS, 3600000);
	...
	public static function SendFilesForAnalyze($files_array = array(), $email_from = 'dontreply@siteguarding.com')
	{
		...
		if (count($files))
		{
            ...
            $zip_file_url = get_site_url().'/wp-content/plugins/wp-antivirus-website-protection-and-firewall/tmp/'.$zip_file;
